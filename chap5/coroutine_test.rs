...
impl Coroutine<()> for MutexCoroutine {
    ...
    fn resume(mut self: Pin<&mut Self>, _arg: ()) -> CoroutineState<Self::Yield, Self::Return> {
        match self.handle.try_lock() {      // try_lock() はノンブロッキング
            Ok(mut handle) => {
                *handle += 1;
            },
            Err(_) => {
                return CoroutineState::Yielded(());     // lock できなかった => Yielded
            }
        }   // lock できた
