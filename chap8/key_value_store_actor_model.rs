...
async fn key_value_actor(mut receiver: Receiver<KeyValueMessage>) {
    ...
    loop {
        match time::timeout(timeout_duration, receiver.recv()).await {
            ...
            Ok(None) => break,
            Err(_) => {     // key_value_actor はまだ生きている、という Heartbeat message を送る
                router_sender.send(RoutingMessage::Heartbeat(ActorType::KeyValue)).await.unwrap();
            }
        };
    }
}
...
async fn writer_actor(mut receiver: Receiver<WriterLogMessage>) -> io::Result<()> {
    ...
    loop {
        match time::timeout(timeout_duration, receiver.recv()).await {
            ...
            Ok(None) => break Ok(()),
            Err(_) => {
                router_sender.send(RoutingMessage::Heartbeat(ActorType::Writer)).await.unwrap();
            }
        };
    }
}
